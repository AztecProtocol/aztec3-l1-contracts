 ...
 * @title TokenPortal
 * @author LHerskind
 * @notice A minimal portal contract that implements a token "bridge" that let a user deposit
 * and withdraw assets from the Rollup.
 * Relies only on the message boxes (inbox & outbox) and the underlying token itself.
 */
